<!DOCTYPE html>
<html>
<meta name="viewport" content="width=device-width, initial-scale=1">


<?php include 'inc/header.php' ?>
<!----------------Content Area------------------>


<div class="w3-container w3-center" style="padding-top:40px; padding-bottom: 20px; width: 100%; align: center;">
<span style="font-size: 30px; color: blue; font-family: arial; font-weight: bold; text-align: middle; padding-top:20px; padding-bottom: 35px;">PLACES TO VISIT IN 3 DAYS</span>
</div>
<div class="w3-container" style="padding-top:10px; padding-bottom: 30px; width: 100%; margin: 10px;">

  <div class="w3-quarter" style="padding-left: 10px; padding-right: 10px;">
  <div class="w3-card-2">
  <img src="images/3daytrip22.jpg" style="width:100%" class="w3-hover-opacity">
    <div class="w3-container">
      <h4 style="text-align: center;">Wayanad</h4>
    </div>
  </div>
</div> 

  <div class="w3-quarter" style="padding-left: 10px; padding-right: 10px;">
  <div class="w3-card-2">
  <img src="images/3daytrip21.jpg" style="width:100%" class="w3-hover-opacity">
    <div class="w3-container">
      <h4 style="text-align: center;">Ooty</h4>
    </div>
  </div>
</div> 

  <div class="w3-quarter" style="padding-left: 10px; padding-right: 10px;">
  <div class="w3-card-2">
 <a href="coorg.php"> <img src="images/3daytrip35.jpg" style="width:100%" class="w3-hover-opacity"></a>
    <div class="w3-container">
      <h4 style="text-align: center;">Coorg</h4>
    </div>
  </div>
</div> 

</div>

<!----------------Content Area------------------>

<script src="js/myscripts.js"></script>

<?php include 'inc/footer.php' ?>


</body>
</html>
